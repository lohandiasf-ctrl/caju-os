...
import { Camera, Check, ChevronUp, Loader2, LogOut, MessageCircle, Send, Users } from 'lucide-react';
...
type Message = { id: number; senderEmail: string; recipientEmail: string; body: string; createdAt: string; readAt: string | null };
...
function ChatDialog({ colleague, onClose }: { colleague: Colleague | null; onClose: () => void }) {
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    if (!user || !colleague || !draft.trim() || sending) return;
    setSending(true); setError('');
    try {
      const response = await fetch('/api/messages', { method: 'POST', headers: { Authorization: `Bearer ${await user.getIdToken()}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ to: colleague.email, body: draft }) });
      ...
      setMessages((current) => [...current, payload.message!]); setDraft('');
