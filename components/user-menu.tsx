...
import { Camera, Check, CheckCheck, ChevronUp, ClipboardList, Clock3, ExternalLink, File, FileAudio, History, Loader2, LogOut, Maximize2, MessageCircle, Mic, MicOff, Minimize2, Paperclip, Phone, PhoneIncoming, PhoneOff, Plus, Search, Send, Signal, Square, UserPlus, Users, X } from 'lucide-react';
...
function IncomingVoiceCall({ invitation, colleagues, onAnswered, onClose, onDecline }: { invitation: VoiceInvitation | null; colleagues: Colleague[]; onAnswered: () => void; onClose: () => void; onDecline: (invitation: VoiceInvitation) => void }) {
  ...
  const startedAtRef = useRef<string | null>(null);
  useEffect(() => { setState('ringing'); setMuted(false); setParticipants(0); setError(''); setMinimized(false); }, [invitation?.roomId]);
  ...
  return <><Dialog open={Boolean(invitation) && state === 'ringing'} onOpenChange={(open) => { if (!open && invitation) onDecline(invitation); }}><DialogContent className="sm:max-w-sm"><DialogHeader><DialogTitle className="flex items-center gap-2"><PhoneIncoming className="size-5 text-emerald-400" />Chamada recebida</DialogTitle><DialogDescription>{caller} convidou você para {invitation?.kind === 'group' ? 'uma reunião de voz' : 'uma chamada de voz'}.</DialogDescription></DialogHeader><div className="flex gap-2"><button type="button" onClick={() => invitation && onDecline(invitation)} className="min-h-11 flex-1 rounded-xl border border-rose-400/40 bg-rose-400/10 px-3 text-sm font-semibold text-rose-200">Recusar</button><button type="button" onClick={() => void accept()} className="min-h-11 flex-1 rounded-xl bg-emerald-500 px-3 text-sm font-semibold text-white">Atender</button></div>{error && <p role="alert" className="text-xs text-rose-300">{error}</p>}</DialogContent></Dialog>
    {active && <section aria-label="Chamada de voz em andamento" className={`fixed bottom-4 right-4 z-[70] rounded-2xl border border-emerald-400/30 bg-card/95 shadow-2xl backdrop-blur-xl transition-[width] motion-reduce:transition-none ${minimized ? 'w-56' : 'w-[min(25rem,calc(100vw-2rem))]'}`}><div className="flex min-h-14 items-center gap-3 p-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-emerald-400/12 text-emerald-300"><Phone className="size-5" /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{invitation?.kind === 'group' ? 'Reunião em andamento' : caller}</p><p className="truncate text-xs text-muted-foreground">{state === 'connecting' ? 'Conectando…' : `${participants + 1} participante(s) · ${qualityLabel(quality)}`}</p></div><button type="button" onClick={() => setMinimized((value) => !value)} className="grid size-10 shrink-0 place-items-center rounded-xl border border-border hover:bg-muted" aria-label={minimized ? 'Expandir chamada' : 'Minimizar chamada'}>{minimized ? <Maximize2 className="size-4" /> : <Minimize2 className="size-4" />}</button></div>{!minimized && <div className="flex gap-2 border-t border-border p-3"><button type="button" disabled={state !== 'connected'} onClick={() => { const next = !muted; setMuted(next); clientRef.current?.setMuted(next); }} className="min-h-11 flex-1 rounded-xl border border-border bg-background px-3 text-xs font-semibold disabled:opacity-50">{muted ? 'Ativar microfone' : 'Silenciar'}</button>{invitation?.kind === 'group' && <button type="button" disabled={state !== 'connected'} onClick={() => setInviteOpen(true)} className="grid size-11 place-items-center rounded-xl border border-border bg-background" aria-label="Adicionar participantes"><UserPlus className="size-4" /></button>}<button type="button" onClick={closeActive} className="min-h-11 flex-1 rounded-xl bg-rose-500 px-3 text-xs font-semibold text-white">Encerrar</button></div>}{error && <p role="alert" className="px-3 pb-3 text-xs text-rose-300">{error}</p>}</section>}
