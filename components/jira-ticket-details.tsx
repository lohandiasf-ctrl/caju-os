...
${chosen.name}` : chosen.name, equipmentTotal: String(Number((totalAtual + chosen.salePriceCents / 100).toFixed(2))).replace('.', ',') }; }); }}><option value="">Adicionar peça do catálogo (soma no valor de equipamentos)...</option>{parts.map((part) => <option key={part.id} value={part.id}>{part.name} · {formatCurrency(part.salePriceCents / 100)}</option>)}</select>}<textarea rows={3} value={form[key] ?? ''} onChange={(event) => setForm((current) => ({ ...current, [key]: event.target.value }))} disabled={Boolean(savingKey)} className="field mt-1.5 min-h-24 text-foreground" /></label>)}</div></div>
    ...
    {preview && <Dialog open onOpenChange={(open) => { if (!open) setPreview(null); }}><DialogContent showCloseButton={false} className="flex flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl"><header className="flex items-center gap-3 border-b border-border p-3"><div className="min-w-0 flex-1"><DialogTitle className="text-sm font-bold">{preview.attachment.filename}</DialogTitle><p className="text-xs text-muted-foreground">{formatBytes(preview.attachment.size)}</p></div><Button type="button" variant="outline" size="sm" onClick={() => void fetchAttachment(preview.attachment, true)}><Download /> Baixar</Button><Button type="button" variant="ghost" size="icon" onClick={() => setPreview(null)} aria-label="Fechar visualização"><X /></Button></header><div className="grid min-h-0 flex-1 place-items-center overflow-hidden bg-black/40 p-3">{preview.attachment.mimeType.startsWith('image/') ? <ImageZoom src={preview.url} alt={preview.attachment.filename} className="h-[76vh] w-full" /> : preview.attachment.mimeType.startsWith('video/') ? <video src={preview.url} controls autoPlay className="max-h-[76vh] max-w-full" /> : preview.attachment.mimeType === 'application/pdf' ? <iframe src={preview.url} title={preview.attachment.filename} className="h-[76vh] w-full rounded-lg bg-white" /> : <div className="max-w-md text-center"><FileText className="mx-auto size-12 text-primary" /><p className="mt-3 text-sm">Este arquivo não possui visualização no navegador.</p><Button type="button" className="mt-4" onClick={() => void fetchAttachment(preview.attachment, true)}><Download /> Baixar arquivo</Button></div>}</div></DialogContent></Dialog>}
  </section>;
}
