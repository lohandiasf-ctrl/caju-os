...
import { Building2, Check, Loader2, RefreshCw, Wrench } from 'lucide-react';
...
export function JiraTicketDetails({ details, user, onUpdated }: { details: Details; user: User; onUpdated: (details: Details) => void }) {
  ...
  function saveField(key: string) {
    ...
    if (value === String(initial[key as keyof typeof initial] ?? '')) return;
    void updateJira({ [key]: value }, key);
  }

  function input(key: string, label: string) {
    return <label key={key} className="text-xs font-semibold text-muted-foreground">{label}<Input value={form[key] ?? ''} onChange={(event) => setForm((current) => ({ ...current, [key]: event.target.value }))} onBlur={() => saveField(key)} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur(); }} disabled={savingKey === key} className="mt-1.5 min-h-11 text-foreground" /></label>;
  }
  ...
  return <section className="space-y-4 rounded-2xl border border-border bg-muted/20 p-4">
    <div className="sticky top-0 z-10 -mx-2 rounded-xl border border-primary/25 bg-background/95 p-3 shadow-lg backdrop-blur-xl">
      ...
      <p className={`mt-2 text-xs ${failed ? 'text-red-300' : 'text-muted-foreground'}`} role={failed ? 'alert' : 'status'}>{message || 'A etapa é alterada aqui e sincronizada imediatamente com o Jira.'}</p>
    </div>
    <div><h3 className="flex items-center gap-2 text-sm font-bold"><Building2 className="size-4 text-primary" />Dados do chamado</h3><p className="mt-1 text-xs text-muted-foreground">Edite diretamente. Cada campo é salvo no Jira ao sair dele.</p></div>
    <div className="grid gap-3 sm:grid-cols-2">{fields.map(([key, label]) => input(key, label))}</div>
    <div className="border-t border-border pt-4"><h3 className="mb-3 flex items-center gap-2 text-sm font-bold"><Wrench className="size-4 text-amber-300" />Resumo técnico no Jira</h3><div className="grid gap-3">{([['identifiedProblem','Problema identificado'],['testsPerformed','Testes feitos'],['partToReplace','Peça a ser trocada']] as const).map(([key,label]) => <label key={key} className="text-xs font-semibold text-muted-foreground">{label}<textarea rows={3} value={form[key] ?? ''} onChange={(event) => setForm((current) => ({ ...current, [key]: event.target.value }))} onBlur={() => saveField(key)} disabled={savingKey === key} className="field mt-1.5 min-h-24 text-foreground" /></label>)}</div></div>
