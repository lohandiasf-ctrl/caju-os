...
// uma única chamada basta no cadastro; 4101 = o código já estava registrado.
...
/** Status atual do envio, ou null quando a chave da TrackingMore não está configurada. */
export async function trackShipment(rawCode: string): Promise<TrackingSnapshot | null> {
  if (!apiKey()) return null;
  const trackingNumber = normalizeTrackingCode(rawCode);

  let courier = knownCourier(trackingNumber);
  if (!courier) {
    ...
    if (!courier) throw new Error(apiErrorMessage(detected.code === 200 ? 4121 : detected.code, detected.payload));
  }
  ...
  if (result.code !== 200) throw new Error(apiErrorMessage(result.code, result.payload));
