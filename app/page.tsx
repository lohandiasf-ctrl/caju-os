...
export default function Home() {
  ...
  return <main className="min-h-screen text-foreground">
    <aside className={`caju-sidebar group/sidebar fixed inset-y-0 left-0 z-40 flex w-[272px] flex-col overflow-hidden border border-sidebar-border bg-sidebar px-4 py-5 transition-[width,transform] duration-200 sm:inset-y-3 sm:left-3 sm:w-[76px] sm:translate-x-0 sm:hover:w-[272px] motion-reduce:transition-none ${menu ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex h-12 items-center gap-3 px-2">
        ...
        <div className="sidebar-label whitespace-nowrap opacity-100 transition-opacity sm:opacity-0 sm:group-hover/sidebar:opacity-100"><div className="text-[15px] font-extrabold tracking-tight">Caju OS</div><div className="text-[10px] font-semibold uppercase tracking-[.16em] text-muted-foreground">Comando operacional</div></div>
      </div>
      <nav className="no-scrollbar mt-5 min-h-0 flex-1 space-y-0.5 overflow-x-hidden overflow-y-auto overscroll-contain pb-3" aria-label="Navegação principal">
        <p className="sidebar-label mb-3 whitespace-nowrap px-3 text-[10px] font-bold uppercase tracking-[.15em] text-muted-foreground opacity-100 transition-opacity sm:opacity-0 sm:group-hover/sidebar:opacity-100">Operação</p>
    ...
    <section className="min-h-screen sm:pl-[92px] xl:pr-8">
      <header className="sticky top-0 z-20 flex h-[68px] items-center gap-3 border-b border-border bg-background/90 px-4 backdrop-blur-xl sm:px-6 lg:px-5">
        <Button variant="ghost" size="icon" className="sm:hidden" aria-label="Abrir menu" onClick={() => setMenu(true)}><Menu /></Button>
