...
import { employeeActivity, jiraSyncJobs, operationalAudit, operationalStores, operationalTasks, operationalVisits, operationalWorkflows, requesterHistory, shipmentTracking, technicians, ticketArchives, ticketSnapshots, voiceCallHistory } from '@/db/schema';
...
export async function GET(request: Request) {
  try {
    ...
    const db = getDb();
    const [workflows, visits, stores, technicianRows, audit, syncJobs, calls, requesters, shipments, tasks, activities, snapshots, archives] = await Promise.all([
