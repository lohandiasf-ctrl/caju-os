...
import { n1TicketAssignments, ticketEvidence } from '@/db/schema';
...
export async function GET(request: Request, context: { params: Promise<{ key: string }> }) {
  try {
    ...
    const [assignment, evidence] = await Promise.all([
      ...
      db.select({ id: ticketEvidence.id, kind: ticketEvidence.kind, name: ticketEvidence.name, mimeType: ticketEvidence.mimeType, createdAt: ticketEvidence.createdAt, uploadedBy: ticketEvidence.uploadedBy }).from(ticketEvidence).where(eq(ticketEvidence.ticketKey, ticketKey)).orderBy(asc(ticketEvidence.createdAt)).all(),
    ]);
...
export async function PUT(request: Request, context: { params: Promise<{ key: string }> }) {
  try {
    ...
    if (action === 'claim') {
      ...
      if (attachments.length) await db.insert(ticketEvidence).values(attachments.map((item) => ({ ticketKey, ...item, uploadedBy: user.email, createdAt: now })));
