...
export async function GET(request: Request) {
  try {
    ...
    if (!bridgeConfigured()) return Response.json({ error: 'O bridge do WhatsApp não está configurado.' }, { status: 503 });
    const state = await fetchBridgeQr();
...
export async function POST(request: Request) {
  try {
    ...
    const upstream = await bridgeFetch('/reset-session', { method: 'POST', signal: AbortSignal.timeout(20_000) });
    const payload = await upstream.json().catch(() => ({})) as { error?: string };
    logSecurityEvent({ request, user, action: 'whatsapp_session_reset', outcome: upstream.ok ? 'allowed' : 'denied', details: { status: upstream.status } });
