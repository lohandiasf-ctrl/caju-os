...
import { accountFromSecret } from '@/lib/server/whatsapp-bridge';

// NUNCA chame o bridge a partir daqui. O bridge encaminha a mensagem dentro do
// handler `messages.upsert` do Baileys e espera esta resposta, uma mensagem por
// vez; uma chamada de volta ao bridge trava o processamento de eventos dele e a
// sessão de criptografia desencontra — o contato passa a ver "Aguardando
// mensagem". Esta rota só grava e responde.
...
export async function POST(request: Request) {
  ...
  await env.DB.batch([
    env.DB.prepare(`INSERT OR IGNORE INTO whatsapp_messages (account, wamid, phone_number_id, contact_phone, contact_name, sender_jid, direction, message_type, body, media_id, quoted_wamid, quoted_body, quoted_name, delivery_status, occurred_at, created_at) VALUES (?, ?, 'bridge', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)`)
      .bind(account, message.wamid, message.contactPhone, message.contactName, message.senderJid, message.direction, message.messageType, message.body, message.mediaId, message.quotedWamid, message.quotedBody, message.quotedName, message.occurredAt, now),
    env.DB.prepare(`
    ...
    `).bind(account, message.contactPhone, message.conversationName, message.ticketKeys, message.occurredAt, now, now),
  ]);
