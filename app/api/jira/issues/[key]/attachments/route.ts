...
const allowedExtensions = /\.(?:png|jpe?g|webp|gif|heic|mp4|mov|webm|avi|pdf|docx?|xlsx?|csv|txt)$/i;
...
export async function POST(request: Request, context: { params: Promise<{ key: string }> }) {
  try {
    ...
    if (files.some((file) => !allowedExtensions.test(file.name))) return Response.json({ error: 'Formato não permitido. Use fotos, vídeos, PDF, Word, Excel, CSV ou TXT.' }, { status: 400 });
