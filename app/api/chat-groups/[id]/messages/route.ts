...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const current = await requireApiUser(request); const groupId = Number((await context.params).id);
    if (!Number.isSafeInteger(groupId) || groupId < 1) return Response.json({ error: 'Grupo não encontrado.' }, { status: 404 });
    const member = await membership(groupId, current.email);
    if (!member) return Response.json({ error: 'Grupo não encontrado.' }, { status: 404 });
    const body = await request.json().catch(() => null) as { body?: unknown; ticketId?: unknown; attachment?: unknown } | null;
    if (!body || typeof body !== 'object') return Response.json({ error: 'Dados da mensagem inválidos.' }, { status: 400 });
    const text = typeof body.body === 'string' ? body.body.trim().slice(0, 2000) : '';
    const ticketId = typeof body.ticketId === 'string' ? body.ticketId.trim().slice(0, 40) || null : null;
    const rawAttachment = body.attachment && typeof body.attachment === 'object' ? body.attachment as { name?: unknown; type?: unknown; data?: unknown } : null;
