  ...
  return <main className="min-h-screen text-foreground">
    <AppNavigation active="finance" open={menu} onOpenChange={setMenu} />
      ...
      <div id="main-content" tabIndex={-1} className="app-main mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="mb-1 text-xs font-bold uppercase tracking-[.14em] text-primary">Gestão financeira</p><h1 className="text-2xl font-semibold tracking-[-.03em] sm:text-3xl">Financeiro</h1><p className="mt-1 text-sm text-muted-foreground">Valores do ticket, spare, repasses e margem em uma visão.</p></div><div className="flex flex-wrap gap-2">{([7, 30, 90] as const).map((days) => <Button key={days} aria-pressed={period === days} size="sm" variant={period === days ? 'secondary' : 'outline'} onClick={() => { setPeriod(days); setPage(1); }}>{days} dias</Button>)}<Button size="sm" onClick={exportCsv} disabled={!rows.length}><Download />Exportar</Button></div></div>
